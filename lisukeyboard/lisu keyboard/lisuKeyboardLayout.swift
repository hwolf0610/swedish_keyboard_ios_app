...
func lisuKeyboardLayout(controller: UIInputViewController, totalWidth: CGFloat, totalHeight: CGFloat, isPortrait: Bool, isIPad: Bool) -> Keyboard {
    ...
    feedbackButton.text = "swedish keyboard"
    ...
    unshiftPage.keyboard = [["q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "å"],
                            ["a", "s", "d", "f", "g", "h", "j", "k", "l", "ö", "ä"],
                            ["shift","z", "x", "c", "v", "b", "n", "m","backspace"],
                            ["123","keyboardchange", "mellanslag",".", "return"]
    ]
    keyboardLayout[MODE_CHANGE_ID.unshift] = unshiftPage
    // Shift
    var shiftPage = page()
    shiftPage.keyboard = [["Q", "ꓪ", "ꓰ", "ꓣ", "ꓔ", "ꓬ", "ꓴ", "ꓲ", "ꓳ", "ꓑ", "Å"],
                          ["ꓮ", "ꓢ", "ꓓ", "ꓝ", "ꓖ", "ꓧ", "ꓙ", "ꓗ", "ꓡ", "Ö", "Ä"],
    ...
    // 123
    var numPage = page()
    numPage.keyboard = [["1", "2", "3", "4", "5", "6", "7", "8", "9", "0","00"],
                        ["/", "\"", "ꓹꓼ", "(", ")", "$", "&", "@","!",":",";"],
                        ["sym","#", "+", "=", "-", "+", "<", ">","backspace"],
                        ["ABC","keyboardchange", "mellanslag",",", "return"]
    ]
    ...
    let MAX_NUM_GAP_HOR: CGFloat = 12 // Max horizontal key is ten, so there are eleven gaps.
    let MAX_NUM_GAP_VER: CGFloat = 5 // There are five rows. So, five gaps
    
    // Determine button width
    // So many magic numbers... :<<<
    let characterSize = CGSize(width: (viewWidth-(GAP_WIDTH * MAX_NUM_GAP_HOR))/11, height: (viewHeight - (GAP_HEIGHT * MAX_NUM_GAP_VER))/4)
    ...
    // Spacebar button
    lastRowKeys.append(charKey.copy(type: .space, keyValue: "mellanslag", width: spacebarSize.width))
    // Period button
